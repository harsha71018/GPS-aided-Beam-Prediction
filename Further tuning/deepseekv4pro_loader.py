...
def run_resource_allocation(pred_beams, true_pwr_matrix, strategy="Max-SNR", n_users=10):
    """
    pred_beams: list/array of predicted beam indices (1D)
    true_pwr_matrix: power matrix, shape (n_samples, n_beams)
    n_users: number of users per scheduling slot
    Noise floor: assumed -90 dBm (1e-9 mW) – change if your system differs.
    """
    ...
    for slot in range(n_slots):
        ...
        for i in range(n_users):
            ...
            # noise floor: 1e-9 mW → -90 dBm. Adjust if needed.
            snr = pwr / 1e-9
...
# combinations: (scen_idx, n_beams, norm_type, noise_placeholder, rep_placeholder)
# noise is always 0, rep is always 1.
...
for scen_idx, n_beams, norm_type, _, rep in combinations:
    ...
    # Preprocess
    pos_norm_clean = func.normalize_pos(pos1, pos2, norm_type)  # currently only min-max on pos2
    ...
    for algo in ai_strategies:
        ...
        if algo == 'KNN':
            ...
            pred_beams = clf.classes_[sorted_idx]  # FIX: Map indices to real beam IDs
            ...
            pred_beams_noisy_enc = clf.predict(x_test_noisy)
            pred_beams_noisy     = le.inverse_transform(pred_beams_noisy_enc)
        ...
        # Inference time measurement (100 samples)
        t0 = time.time()
        if algo == 'NN':
        ...
        # Noisy accuracy and robustness drop
